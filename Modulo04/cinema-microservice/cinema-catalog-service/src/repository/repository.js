...
async function getAllCities(){
    ...
    .find({})
    .project({ cidade: 1, uf: 1, pais: 1 })//irá fazer com que retorne somente o que precisamos 1 - retorna esse campo somente 0 - retorna todos menos esse
    .toArray();
}
...
async function getMoviesByCinemaId(cinemaId){
    ...
    const group = await db.collection('cinemaCatalog')
             .aggregate([
                 { $match: { "cinemas._id": objCinemaId } }, //filtro da minha agregação
                 { $unwind:  "$cinemas"  }, //vai entrar nos niveis mais baixos e trazer eles
...
async function getMoviesByCityId(cityId){
    ...
    const group = await db.collection('cinemaCatalog')
             .aggregate([
                 { $match: { "_id": objCityId } }, //filtro da minha agregação
                 { $unwind:  "$cinemas"  }, //vai entrar nos niveis mais baixos e trazer eles
                 { $unwind:  "$cinemas.salas" },
                 { $unwind:  "$cinemas.salas.sessoes" },
                 { $group: { _id: { titulo: "$cinemas.salas.sessoes.filme", idFilme: "$cinemas.salas.sessoes.idFilme" }} }
